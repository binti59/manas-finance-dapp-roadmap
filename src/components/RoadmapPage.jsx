...
const RoadmapPage = () => {
  ...
  const updateTaskProgress = (quarterId, taskId, newProgress) => {
    setRoadmapData(prev => ({
      ...prev,
  ...
  const TaskCard = ({ quarter, task, isAdmin = false }) => {
    const categoryColor = categoryColors[task.category] || 'bg-gray-600'
      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10 hover:border-white/20 transition-all duration-300">
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1">
            ...
            <span className={`inline-block px-2 py-1 rounded-full text-xs text-white ${categoryColor}`}>
              {task.category}
            </span>
          </div>
          <div className="flex items-center space-x-2">
            ...
            {isAdmin && (
              <Button
                ...
                onClick={() => setEditingTask({ ...task, quarterId: quarter.id })}
              >
                <Edit size={16} />
              </Button>
            )}
    ...
    <div className="min-h-screen px-4 py-8">
      ...
      {showAdminPanel && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 rounded-2xl p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-white/20">
            <div className="flex justify-between items-center mb-6">
            ...
            <div className="space-y-6">
              {roadmapData.quarters.map(quarter => (
                <div key={quarter.id}>
                  <h3 className="text-xl font-semibold text-white mb-4">{quarter.name}</h3>
